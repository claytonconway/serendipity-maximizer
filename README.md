...
4. Open `app/discovery-board.bundled.jsx` as a React artifact to manage discoveries. This is the self-contained build (all `app/lib/*.mjs` imports inlined, generated by `scripts/build-board.mjs`) — it renders as a React artifact, self-seeds sample data, and persists via the storage adapter (`window.storage`, falling back to `localStorage`). Edit the source in `app/discovery-board.jsx` + `app/lib/`, then regenerate with `node scripts/build-board.mjs`.
